...
- Pionex: **spot only — NO futures martingale** (absence finding). Volume scale (Std) / share split (Simple); price scale from LAST FILL; TP % rebound from avg entry, sells all; auto-cycle; no SL documented; no max-orders cap published. Trailing mode (waits for bottom + 0.5% rebound); TBT/QFL signals; DIY per-order shares.
